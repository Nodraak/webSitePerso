...
*******************************************************************************/
	...
	while ($data_thread = $req_thread->fetch())
	{
		...
		$urlFirstPage = 'index.php?page=message&id='.$forum->get_id().'&offset=1';
		$urlLastPage = 'index.php?page=message&id='.$forum->get_id().'&offset=last';

		echo '<div class="thread"><table>
				<tr>
					<td class="title"><a href="'.$urlFirstPage.'">'.$forum->get_title().'</a></td>
					<td class="time time_activity">'.$forum->get_nbMessage().' messages, <a href="'.$urlLastPage.'">dernière activité</a> le ' . date('d/m/Y à H\hi', $forum->get_activity()) . '</td>
				</tr><tr>
